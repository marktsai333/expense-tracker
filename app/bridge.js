...
(() => {
  ...
  csvInput.onchange=async()=>{const file=csvInput.files?.[0];csvInput.value='';if(!file)return;if(file.size>20e6)return showToast('檔案超過 20 MB');const text=await file.text();showConfirmAlert('匯入這份 CSV？相同交易會略過，現有交易會保留。會先保存目前的雲端資料。',()=>run(async()=>{if(window.cloudSync?.activeCode)await window.cloudSync.createBackup('before-csv-import');await utils.importCSV(store,text);},()=>showToast('CSV 已匯入')));};
